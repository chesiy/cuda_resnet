...
void calc_U(float* kernel, float*U, int in_channels, int out_channels){
    ...
    // U: in_channels x out_channels x 36
    // float *U_tmp = new float[out_channels * in_channels * 36];
    for(int k=0; k<out_channels; k++){
        for(int c=0; c<in_channels; c++){
            ...
            for(int i=0; i<36; i++){
                ...
                U[c*out_channels*36 + k*36 + i] = u[i];
            }
        }
    }
}
...
__global__ void calc_V(float* inp, float* V, int P, int batch_size, int in_channels, int in_numrow, int in_numcol, int tile_numrow, int tile_numcol){
    ...
    V[cur_channel*P*36 + (blockIdx.x*tile_numrow*tile_numcol + blockIdx.y)*36 + threadIdx.y*6 + threadIdx.x] = tmp;

    // printf("%d %d %d %d %f %f %f\n", cur_channel, cur_row, cur_col, cur_channel*P*16 + blockIdx.x*in_numrow*in_numcol*4+blockIdx.y*16 + threadIdx.y*4 + threadIdx.y, 
    //     Btd_shared[threadIdx.y][threadIdx.y], inp_shared[threadIdx.y][threadIdx.y], tmp);
...
__global__ void calc_UV_2(float* U, float* V, float* out, int out_channels, int in_channels, int P){
    ...
    for(int m=0; m<iter_num; m++){
        ...
        // U[row, m*mm_tilewidth+threadIdx.y, place_in_36]
        if(read_col < in_channels)
            Uds[threadIdx.z][threadIdx.y][place_in_36] = U[read_col*out_channels*36 + row*36 + place_in_36]; 
        else
...
__global__ void calc_AtmA(float* M, float* out, int out_channels, int P, int out_numrow, int out_numcol, int tile_num, int tile_numrow, int tile_numcol){
    ...
    // if(threadIdx.x >= 6 && threadIdx.y >= 6) return;

    // TODO: This memory may be optimized, too; only 6*6 is enough
    __shared__ float m[6][6];
    __shared__ float Atm[4][6];
    m[threadIdx.y][threadIdx.x] = M[cur_channel*P*36 + (cur_batch*tile_num+blockIdx.z)*36 + threadIdx.y*6 + threadIdx.x];
    __syncthreads();

    if(threadIdx.y > 3) return; // valid operation?

    switch(threadIdx.y){
        case 0:
            Atm[threadIdx.y][threadIdx.x] = m[0][threadIdx.x] + m[1][threadIdx.x] + m[2][threadIdx.x] + 
                                                    m[3][threadIdx.x] + m[4][threadIdx.x];
            break;
        case 1:
            Atm[threadIdx.y][threadIdx.x] = m[1][threadIdx.x] - m[2][threadIdx.x] + 2*m[3][threadIdx.x] - 2*m[4][threadIdx.x];
            break;
        case 2:
            Atm[threadIdx.y][threadIdx.x] = m[1][threadIdx.x] + m[2][threadIdx.x] + 4*m[3][threadIdx.x] + 4*m[4][threadIdx.x];
            break;
        case 3:
            Atm[threadIdx.y][threadIdx.x] = m[1][threadIdx.x] - m[2][threadIdx.x] + 8*m[3][threadIdx.x] -
                                                8*m[4][threadIdx.x] + m[5][threadIdx.x];
            break;
    }
    __syncthreads();

    if(threadIdx.x > 3) return;

    float tmp = 0;
    switch(threadIdx.x){
        case 0:
            tmp = Atm[threadIdx.y][0] + Atm[threadIdx.y][1] + Atm[threadIdx.y][2] + Atm[threadIdx.y][3] + Atm[threadIdx.y][4];
            break;
        case 1:
            tmp = Atm[threadIdx.y][1] - Atm[threadIdx.y][2] + 2*Atm[threadIdx.y][3] - 2*Atm[threadIdx.y][4];
            break;
        case 2:
            tmp = Atm[threadIdx.y][1] + Atm[threadIdx.y][2] + 4*Atm[threadIdx.y][3] + 4*Atm[threadIdx.y][4];
            break;
        case 3:
            tmp = Atm[threadIdx.y][1] - Atm[threadIdx.y][2] + 8*Atm[threadIdx.y][3] - 8*Atm[threadIdx.y][4] + Atm[threadIdx.y][5];
            break;
    }
    // __syncthreads();

    // out[cur_batch, cur_channel, cur_tilerow*4+threadIdx.y, cur_tilecol*4+threadIdx.y]
    int now_row = 4*cur_tilerow+threadIdx.y;
    int now_col = 4*cur_tilecol+threadIdx.x;
    if(now_row < out_numrow && now_col < out_numcol){
        // printf("%d %d \n", now_col, now_row);
        // if(now_col>4 && now_row>4) printf("%d %d %f\n", now_col, now_row, tmp);
        out[cur_batch*out_channels*out_numrow*out_numcol + cur_channel*out_numrow*out_numcol + 
...
{  
    ...
    int batch_size=2, out_row=56, out_col=56, tile_numrow=14, tile_numcol=14;
    int tile_num = 14*14, P = 14*14*2;
    ...
    cudaMemcpy(d_U, U, sizeof(float) * 16*8*36, cudaMemcpyHostToDevice);
    ...
    for(int i=0; i<100000;i++){
        calc_V<<<dim3(batch_size, tile_num, in_channels), dim3(6, 6)>>>(d_inp, d_V, P, batch_size, in_channels, inp_row, inp_col, tile_numrow, tile_numcol);
        // calc_UV<<<dim3((out_channels+mm_tilewidth-1)/mm_tilewidth, (P+mm_tilewidth-1)/mm_tilewidth, 36), dim3(mm_tilewidth, mm_tilewidth)>>>
        //         (d_U, d_V, d_UV, out_channels, in_channels, P);
        calc_UV_2<<<dim3((P+mm_tilewidth-1)/mm_tilewidth, (out_channels+mm_tilewidth-1)/mm_tilewidth), dim3(36, mm_tilewidth, mm_tilewidth)>>>
                (d_U, d_V, d_UV, out_channels, in_channels, P);
        calc_AtmA<<<dim3(out_channels, batch_size, tile_num), dim3(6, 6)>>>(d_UV, d_out, out_channels, P, out_row, out_col, tile_num, tile_numrow, tile_numcol);
    }
