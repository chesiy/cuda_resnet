...
namespace im2col_trans {
    ...
    __global__ void im2col_CHW(float* A, float* out, int kernel_size, int in_channel, int in_numrow, int in_numcol,
        ...
        // out: batch_size x (out_rownum * out_colnum) x (in_channel * K * K)
        ...
        // int idx = blockIdx.x * row_len * col_len + cur_mat_col * row_len + cur_mat_row;
    ...
    __global__ void matmul_alloc_transpose(float* K_mm, float* Inp_mm, float* out, int batch_size, int out_channel, int out_numrow,
        ...
        int row = blockIdx.z * mm_tilewidth + threadIdx.y;
        ...
        for (int m = 0; m < iter_num; m++) {
            ...
            if (read_col < l2) {
                Ks[threadIdx.y][threadIdx.x] = K_mm[row + read_col * l1];
            } else {
            ...
            if (read_row < l2) {
                Is[threadIdx.y][threadIdx.x] = Inp_mm[blockIdx.x * l2 * l3 + col + read_row * l3];
            } else {
...
#pragma unroll
    ...
    __global__ void matmul_alloc_transpose_bias(float* K_mm, float* Inp_mm, float* out, float* bias, int batch_size, int out_channel, int out_numrow,
                                           int out_numcol, int l1, int l2, int l3) {
        // K_mm: l2 x l1, inp_mm: l2 x l3,
        // l1=out_channel, l2=in_channel x kernel_size x kernel_size, l3=out_numrow x out_numcol
        // block: batch x floor(l3 / mm_tile_width) x floor(l1 / mm_tile_width), thread: mm_tile_width x mm_tile_width
...
#pragma unroll
    ...
    __global__ void matmul_alloc_transpose_bias_relu(float* K_mm, float* Inp_mm, float* out, float* bias, int batch_size, int out_channel, int out_numrow,
                                                int out_numcol, int l1, int l2, int l3) {
        // K_mm: l2 x l1, inp_mm: l2 x l3,
        // l1=out_channel, l2=in_channel x kernel_size x kernel_size, l3=out_numrow x out_numcol
        // block: batch x floor(l3 / mm_tile_width) x floor(l1 / mm_tile_width), thread: mm_tile_width x mm_tile_width
...
#pragma unroll
        ...
        if (row < l1 && col < l3) {
            ...
            // int cur_out_row = col / out_numrow, cur_out_col = col % out_numrow;
            int idx = blockIdx.x * l1 * l3 + row * l3 + col;
            out[idx] = ((p_value + bias[row])>0)?(p_value + bias[row]):0;
        }
    }
