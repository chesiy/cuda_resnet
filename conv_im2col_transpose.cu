...
__global__ void im2col_CHW_transpose(float* A, float* out, int kernel_size, int in_channel, int in_numrow, int in_numcol, 
...
{  
    ...
    for(int i=0; i<10000;i++){
        im2col_CHW_transpose<<<dim3(batch_size, out_row*in_channels, kernel_size*kernel_size), dim3(out_col)>>>
        ...
        matmul_alloc_transpose<<<
            dim3(batch_size, (out_row*out_col+mm_tilewidth-1)/mm_tilewidth, (out_channels+mm_tilewidth-1)/mm_tilewidth), 
            dim3(mm_tilewidth, mm_tilewidth)
            >>>(d_kernel, d_inp_col, d_out, batch_size, out_channels, inp_row, inp_col, out_channels, 
            (in_channels*kernel_size*kernel_size), (out_row*out_col));
    }
