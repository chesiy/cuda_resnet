...
__global__ void calc_UV_2(float* U, float* V, float* out, int out_channels, int in_channels, int P){
    // block: floor(l1 / mm_tile_width) x floor(l2 / mm_tile_width), threads: mm_tile_width x mm_tile_width x 16
    // U: out_channels x in_channels x 16, V: in_channels x P x 16, out: out_channels x P x 16
    __shared__ float Uds[mm_tilewidth][mm_tilewidth][16];
    __shared__ float Vds[mm_tilewidth][mm_tilewidth][16];
    
    int row = blockIdx.x * mm_tilewidth + threadIdx.x;
    int col = blockIdx.y * mm_tilewidth + threadIdx.y;
    int place_in_16 = threadIdx.z;
    float p_value = 0;
    ...
    for(int m=0; m<iter_num; m++){
        ...
        int read_col = m*mm_tilewidth+threadIdx.y;
        if(read_col < in_channels)
            Uds[threadIdx.x][threadIdx.y][place_in_16] = U[row*in_channels*16 + read_col*16 + place_in_16]; 
        else
            Uds[threadIdx.x][threadIdx.y][place_in_16] = 0;
        
        int read_row = m*mm_tilewidth+threadIdx.x;
        // V[m*mm_tilewidth+threadIdx.x, col, place_in_16]
        if(read_row < in_channels)
            Vds[threadIdx.x][threadIdx.y][place_in_16] = V[read_row*P*16 + col*16 + place_in_16];
        else
            Vds[threadIdx.x][threadIdx.y][place_in_16] = 0;
        
        // printf("%d %d\n", read_col, read_row);
        __syncthreads();

        for(int k=0; k<mm_tilewidth; k++){
            p_value += Uds[threadIdx.x][k][place_in_16] * Vds[k][threadIdx.y][place_in_16];
        }
        __syncthreads();
    }
